...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'], $_POST['phone'])) {
  ...
  $currentDate = date ('Y-m-d');// format date : yyyy-mm-dd
  
  //code to insert data into database
  $sql = "INSERT INTO myviData ( prosName, prosNum, prosLocation, saName, curDate ) VALUES ( '$name', '$phone', '$selected_location', '$rotated_link', '$currentDate' )";
  ...
  //Get the current rotation index from the session 
  session_start();
